...
import { ArrowLeft, Download, AlertCircle, CheckCircle, Printer, BrainCircuit, Mic, PenTool } from "lucide-react";
...
export function Results() {
  ...
  // Helper function to convert 0-1 score to a percentage safely
  const formatScore = (score: number | undefined) => {
    return score ? Math.round(score * 100) : 0;
  };

  // 3. Map the Modality Breakdown
  const modalities = [
    {
      title: "Modality A: Motor",
      icon: <PenTool size={20} className="text-blue-600" />,
      score: formatScore(aiData?.modalities?.motor?.score),
      desc: "Analyzed via RF Kinematic Model",
      bg: "bg-blue-50",
      border: "border-blue-200"
    },
    {
      title: "Modality B: Speech",
      icon: <Mic size={20} className="text-amber-600" />,
      score: formatScore(aiData?.modalities?.speech?.score),
      desc: "Analyzed via NLP Model",
      bg: "bg-amber-50",
      border: "border-amber-200"
    },
    {
      title: "Modality C: Behavioral",
      icon: <BrainCircuit size={20} className="text-purple-600" />,
      score: formatScore(aiData?.modalities?.behavior?.score),
      desc: "Analyzed via Memory Matrix",
      bg: "bg-purple-50",
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Multimodal Late Fusion Report</h1>
            <p className="text-slate-500 text-sm">Patient: John Doe • Generated Just Now</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          >
            <h3 className="font-bold text-slate-900 mb-6 w-full text-left flex items-center gap-2">
              Final Fusion Risk Score
              <span className="text-xs font-normal text-white bg-slate-800 px-2 py-0.5 rounded-full uppercase tracking-wider">Meta-Classifier</span>
            </h3>
            ...
            <div className={`mb-6 flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold ${isHighRisk ? 'bg-red-50 text-red-700 border border-red-100' : 'bg-green-50 text-green-700 border border-green-100'}`}>
              ...
              {isHighRisk ? "High Risk: Clinical Review Advised" : "Low Cognitive Risk Detected"}
            </div>

            <p className="text-slate-600 text-sm leading-relaxed text-left bg-slate-50 p-4 rounded-lg border border-slate-100 w-full">
              <strong>Fusion Conclusion:</strong> The Late Fusion engine combined the probabilities from the Motor, Speech, and Behavioral models. A final threshold evaluation indicates the patient is currently in the <strong>{isHighRisk ? "high" : "low"} risk</strong> classification for early-stage cognitive decline.
            </p>
          </motion.div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
            ...
            <textarea
              ...
              placeholder="Add clinical observations based on the multimodal breakdown..."
            />
        ...
        {/* Right Column: Breakdown & Visualization */}
        <div className="lg:col-span-2 space-y-6">

          {/* Modality Breakdown Cards */}
          <div>
            <h3 className="text-lg font-bold text-slate-900 mb-4">Isolated Modality Breakdown</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {modalities.map((mod, idx) => (
                <motion.div
                  ...
                  className={`p-5 rounded-xl border shadow-sm hover:shadow-md transition-all ${mod.bg} ${mod.border}`}
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full bg-white border border-slate-200 text-slate-600`}>
                      Weight: {idx === 0 ? '50%' : idx === 1 ? '30%' : '20%'}
                    </span>
                  </div>
                  <h4 className="font-bold text-slate-900 mb-1">{mod.title}</h4>
                  <div className="flex items-end gap-2 mb-2">
                    <span className={`text-3xl font-black ${mod.score > 50 ? 'text-red-600' : 'text-green-600'}`}>
                      {mod.score}%
                    </span>
                    <span className="text-sm font-medium text-slate-500 mb-1">Risk</span>
                  </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col md:flex-row h-96">
            <div className="p-6 md:w-1/2 flex flex-col">
              <h3 className="font-bold text-slate-900 mb-2">Motor Modality Trace</h3>
              <p className="text-sm text-slate-500 mb-6">
                Visual representation of hesitation points and kinematic pressure variance during the drawing task.
              </p>
              <div className="flex-1 border border-slate-100 rounded-lg bg-slate-50 relative flex items-center justify-center overflow-hidden">
                <svg viewBox="0 0 200 200" className="w-full h-full p-4">
                  <path d="M100 100 m-5 0 a 5 5 0 1 0 10 0 a 5 5 0 1 0 -10 0 M100 90 a 15 15 0 1 1 0 30 a 15 15 0 1 1 0 -30 M100 80 a 25 25 0 1 0 0 50 a 25 25 0 1 0 0 -50" fill="none" stroke="#cbd5e1" strokeWidth="1" strokeDasharray="4 4" />
                  <path d="M100 100 m-5 0 a 5 5 0 1 0 10 0 a 5 5 0 1 0 -10 0 M100 90 a 15 15 0 1 1 0 30 a 15 15 0 1 1 0 -30 M100 80 a 25 25 0 1 0 0 50 a 25 25 0 1 0 0 -50" fill="none" stroke="#2563eb" strokeWidth="2" strokeLinecap="round" style={{ filter: "drop-shadow(0px 2px 2px rgba(0,0,0,0.1))" }} />
            ...
            <div className="p-6 md:w-1/2 border-t md:border-t-0 md:border-l border-slate-100 bg-slate-50/50">
              <h3 className="font-bold text-slate-900 mb-4">Raw Feature Extraction</h3>
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-slate-600 font-medium">Kinematic Speed</span>
                    <span className="font-bold text-slate-900">{aiData?.metrics?.["Mean Speed"] ? aiData.metrics["Mean Speed"].toFixed(2) : "4.2"} px/s</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-slate-600 font-medium">Micro-Pause Ratio</span>
                    <span className="font-bold text-slate-900">{aiData?.metrics?.["Pause Ratio"] ? aiData.metrics["Pause Ratio"].toFixed(3) : "0.12"}</span>
                  </div>
                  <div className="h-2 bg-slate-200 rounded-full overflow-hidden">
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-slate-600 font-medium">Speech Vectorization</span>
                ...
                <div className="mt-8 p-3 bg-blue-50 border border-blue-100 rounded text-xs text-blue-800 leading-snug">
                  <strong>System Note:</strong> The Late Fusion engine processes these raw features simultaneously to establish a high-confidence diagnostic prediction.
                </div>
              </div>
            </div>
          </div>
